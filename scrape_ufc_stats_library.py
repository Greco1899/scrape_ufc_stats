...
def organise_fight_results(results_from_soup, fight_results_column_names):
    ...
    fight_results_clean.extend(
        [re.sub("^(.+?): ?", "", text) for text in results_from_soup[5:]]
    )
...
def organise_fight_stats(stats_from_soup):
    ...
    for name, stats in itertools.groupby(
        stats_from_soup, lambda x: x == stats_from_soup[0]
    ):
...
def convert_fight_stats_to_df(
    clean_fighter_stats, totals_column_names, significant_strikes_column_names
):
    """
    ...
    if len(clean_fighter_stats) == 0:
        ...
        significant_strikes_df.loc[len(significant_strikes_df)] = [np.nan] * len(
            list(significant_strikes_df)
        )
    ...
    # if list of stats is no empty
        ...
        for round in range(number_of_rounds):
            ...
            totals_df.loc[len(totals_df)] = [
                "Round " + str(round + 1)
            ] + clean_fighter_stats[round + 1]
            # append each round of significant strike stats from second half of list to significant_strikes_df
            significant_strikes_df.loc[len(significant_strikes_df)] = [
                "Round " + str(round + 1)
            ] + clean_fighter_stats[round + 1 + int((len(clean_fighter_stats) / 2))]
...
def combine_fighter_stats_dfs(fighter_a_stats_df, fighter_b_stats_df, soup):
    ...
    for tag in soup.find_all("a", class_="b-link b-fight-details__person-link"):
        ...
        fighter_urls.append(tag.get("href"))

    fighter_a_stats_df["fighterURL"] = fighter_urls[0]
    fighter_b_stats_df["fighterURL"] = fighter_urls[1]
...
def parse_organise_fight_results_and_stats(
    soup,
    url,
    ...
    # convert list of fighter stats into a structured dataframe
    fighter_a_stats_df = convert_fight_stats_to_df(
        fighter_a_stats_clean, totals_column_names, significant_strikes_column_names
    )
    fighter_b_stats_df = convert_fight_stats_to_df(
        fighter_b_stats_clean, totals_column_names, significant_strikes_column_names
    )
    # combine fighter stats into one
...
def generate_alphabetical_urls():
    ...
    for character in list(string.ascii_lowercase):
        list_of_alphabetical_urls.append(
            "http://ufcstats.com/statistics/fighters?char=" + character + "&page=all"
        )
...
def parse_fighter_details(soup, fighter_details_column_names):
    ...
    fighter_details_df = pd.DataFrame(
        fighter_details, columns=fighter_details_column_names
    )
...
# reorder columns
def move_columns(df, cols_to_move=None, ref_col="", place=""):
    """
    ...
    if place == "after":
        ...
        raise ValueError('place must be "before" or "after"')
